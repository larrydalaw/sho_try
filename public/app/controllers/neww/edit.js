angular.module('myApp').controller('EditNewwController', 
  ['$scope', '$routeParams', '$location', '$translate', '$timeout', 'UserErrorService', 'NavigationService', 'EntityUtilService', 'SecurityService', 'NewwService', 
  function($scope, $routeParams, $location, $translate, $timeout, UserErrorService, NavigationService, EntityUtilService, SecurityService, NewwService) {
	...
	$scope.add = function () {
		...
		NewwService.add(dataToServer($scope.obj)).then(function (httpResponse) {
			if($scope.parent) {
	...
	$scope.update = function () {
		$scope.uiWorking = true;
		NewwService.update(dataToServer($scope.obj)).then(function () { //httpResponse
				returnBack();
	...
	$scope.delete = function () {
		$scope.uiWorking = true;
		NewwService.delete($scope.obj._id).then(returnBack, errorHandlerDelete, progressNotify);
	};
	...
	$scope.gotoEdit = function() {
		$location.path('/neww/edit/' + $routeParams.id);		
	};

	$scope.gotoDelete = function() {
		$location.path('/neww/delete/' + $routeParams.id);		
	};
	...
	function init() {
		...
		if ($routeParams.id) {
			NewwService.getDocument($routeParams.id).then(loadData, errorHandlerLoad);		
		}
